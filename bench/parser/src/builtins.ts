...
import { ReportMode, Resolver } from "./resolver";

import { CommonFlags, CommonNames, Feature, featureToString, TypeinfoFlags } from "./common";
...
function builtin_resolveReturnOfType(ctx: BuiltinTypesContext): Type | null {
  ...
  if (reportMode == ReportMode.Report) {
    resolver.error(DiagnosticCode.Type_0_has_no_call_signatures, typeArgumentNode.range, typeArgument.toString());
  }
  return null;
}
...
function builtin_isString(ctx: BuiltinFunctionContext): ExpressionRef {
  ...
  return reifyConstantType(
    ctx,
    module.i32(classReference && classReference.isAssignableTo(compiler.program.stringInstance) ? 1 : 0)
  );
}
...
function builtin_isArray(ctx: BuiltinFunctionContext): ExpressionRef {
  ...
  return reifyConstantType(
    ctx,
    module.i32(classReference && classReference.extendsPrototype(compiler.program.arrayPrototype) ? 1 : 0)
  );
}
    ...
    switch (type.kind) {
      ...
      case TypeKind.F32: {
        if (getExpressionId(arg0) == ExpressionId.LocalGet) {
          return module.binary(
            BinaryOp.EqF32,
            module.binary(BinaryOp.SubF32, arg0, module.local_get(getLocalGetIndex(arg0), TypeRef.F32)),
            module.f32(0)
          );
        }
        let flow = compiler.currentFlow;
      ...
      case TypeKind.F64: {
        if (getExpressionId(arg0) == ExpressionId.LocalGet) {
          return module.binary(
            BinaryOp.EqF64,
            module.binary(BinaryOp.SubF64, arg0, module.local_get(getLocalGetIndex(arg0), TypeRef.F64)),
            module.f64(0)
          );
        }
        let flow = compiler.currentFlow;
...
function builtin_rem(ctx: BuiltinFunctionContext): ExpressionRef {
  ...
  let left = operands[0];
  let arg0 = typeArguments
    ? compiler.compileExpression(left, typeArguments[0], Constraints.ConvImplicit)
    : compiler.compileExpression(operands[0], Type.auto);
  let type = compiler.currentType;
...
function builtin_atomic_load(ctx: BuiltinFunctionContext): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Threads) | checkTypeRequired(ctx, true) | checkArgsOptional(ctx, 1, 2))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments;
  let contextualType = ctx.contextualType;
  let type = typeArguments![0];
  let outType =
    type.isIntegerValue && contextualType.isIntegerValue && contextualType.size > type.size ? contextualType : type;
  if (!type.isIntegerValue) {
    compiler.error(
      DiagnosticCode.Operation_0_cannot_be_applied_to_type_1,
      ctx.reportNode.typeArgumentsRange,
...
function builtin_atomic_binary(ctx: BuiltinFunctionContext, op: AtomicRMWOp, opName: string): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Threads) | checkTypeRequired(ctx, true) | checkArgsOptional(ctx, 2, 3))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments;
  let contextualType = ctx.contextualType;
...
function builtin_atomic_cmpxchg(ctx: BuiltinFunctionContext): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Threads) | checkTypeRequired(ctx, true) | checkArgsOptional(ctx, 3, 4))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments;
  let contextualType = ctx.contextualType;
...
function builtin_atomic_fence(ctx: BuiltinFunctionContext): ExpressionRef {
  ...
  let module = compiler.module;
  compiler.currentType = Type.void;
  if (checkFeatureEnabled(ctx, Feature.Threads) | checkTypeAbsent(ctx) | checkArgsRequired(ctx, 0))
    return module.unreachable();
...
function builtin_function_call(ctx: BuiltinFunctionContext): ExpressionRef {
  ...
  if (
    checkTypeAbsent(ctx) | checkArgsOptional(ctx, 1 + signature.requiredParameters, 1 + signature.parameterTypes.length)
  ) {
    compiler.currentType = returnType;
...
function builtin_v128_extract_lane(ctx: BuiltinFunctionContext): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Simd) | checkTypeRequired(ctx, true) | checkArgsRequired(ctx, 2))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments!;
  let type = typeArguments[0];
...
function builtin_v128_load_splat(ctx: BuiltinFunctionContext): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Simd) | checkTypeRequired(ctx, true) | checkArgsOptional(ctx, 1, 3))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments!;
  let type = typeArguments[0];
...
function builtin_v128_load_lane(ctx: BuiltinFunctionContext): ExpressionRef {
  let compiler = ctx.compiler;
  let module = compiler.module;
  if (checkFeatureEnabled(ctx, Feature.Simd) | checkTypeRequired(ctx, true) | checkArgsOptional(ctx, 3, 5))
    return module.unreachable();
  let operands = ctx.operands;
  let typeArguments = ctx.typeArguments!;
  let type = typeArguments[0];
      ...
      for (let _values = Map_values(members), j = 0, l = _values.length; j < l; ++j) {
        ...
        body.push(
          ...
          module.call(
            visitInstance.internalName,
            [
              module.load(sizeTypeSize, false, module.local_get(0, sizeTypeRef), sizeTypeRef, fieldOffset), // value
              module.local_get(1, TypeRef.I32), // cookie
            ],
            TypeRef.None
          )
